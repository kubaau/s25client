    ...
    if(GetSize() == BuildingQuality::Castle || GetSize() == BuildingQuality::Harbor)
    {
        for(const Direction i : {Direction::West, Direction::NorthWest, Direction::NorthEast})
        {
            ...
            if(type == BuildingType::Headquarters)
            {
                ...
                // Don't replace nearby static objects or trees. Needed for "build headquarters" cheat to work the same
                // as in S2 since it doesn't need more bq than a hut. This situation shouldn't happen any other way
                // (can't normally build big buildings right next to static objects or trees). Trees which remain
                // because of this will be replaced by extensions instead of stumps if they are cut while still right
                // next to the HQ (see noTree::HandleEvent/State::FallingFallen).
                if(neighborNoType == NodalObjectType::Object || neighborNoType == NodalObjectType::Tree)
                    continue;
            }

            world->DestroyNO(neighbor, /*checkExists*/ false);
