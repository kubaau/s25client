      ...
      gwv(gwv), selectedPt(selectedPt), mousePosAtOpen_(mousePos)
{
    /*
        ...
        TAB_BUILD_HQ 1 = Build headquarters cheat
    */
    ...
    if(tabs.watch)
    {
        if(gi.GI_GetCheats().canBuildHQ(selectedPt))
        {
            glArchivItem_Bitmap* buildImg = LOADER.GetImageN("io", 18);
            ctrlGroup* group = main_tab->AddTab(buildImg, _("Build headquarters"), TAB_BUILDHQ);
...
void iwAction::Msg_Group_ButtonClick(const unsigned /*group_id*/, const unsigned ctrl_id)
{
    switch(GetCtrl<ctrlTab>(0)->GetCurrentTab())
    {
        ...
        case TAB_BUILDHQ:
        {
            Msg_ButtonClick_TabBuildHQ(ctrl_id);
        }
        break;
    }
}
...
void iwAction::Msg_ButtonClick_TabBuildHQ(const unsigned ctrl_id)
{
    switch(ctrl_id)
    {
        case 1:
            gi.GI_GetCheats().buildHQ(selectedPt);
            Close();
            break;
    }
}
